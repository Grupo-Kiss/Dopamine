...
6. **Credits are reachable** — Start Page and Game Over (and About if added). Loading splash is **not** the sole credits surface.
...
| Surface | Requirement |
| --- | --- |
| **Start Page** | Primary “Credits” / About control before PLAY |
| **Game Over** | Same info blocks adapted beside stats + PLAY AGAIN (`09`) |
| Optional About | If settings exist later, link here too |

### Credits screen content (minimum)

1. **Game** — title, short one-liner, studio / authors as you define.
2. **Third-party media** — scrollable list built from the credits store (title / creator / license / source).
3. **Fonts / libraries** — npm and font licenses that require notice (generated or hand-maintained list).
4. **Provider notices** — when a provider was used this build/session, show the required courtesy line (e.g. Pixabay / Pexels encouragement).
5. **Disclaimer** — satire; not affiliated with any real social or streaming platform.

English UI (`09`). Keep readable under Accessibility Mode (no essential info only in motion).

### In-Match attribution

- Do **not** clutter gameplay with permanent watermark bars.
- Optional subtle per-item credit on Wave/Echo detail chrome is allowed if a license demands visible credit **during** playback — prefer satisfying that via the Credits screen unless the license text requires on-screen credit.
- Pixabay “show where results come from” applies when **displaying search results**; in-game playback of cached items still lists those items in Credits.
...
- [ ] Credits reachable from Start Page and Game Over
...
- Tests (`11`): preload rejects missing license; Credits route/components render a fixture list — full license review remains human.
...
- Credits entry points exist on Start Page and Game Over (splash alone is insufficient).
